...
function toggleCronograma(btn) {
    ...
    .then(data => {
        ...
        const elAnt = document.getElementById(statIds[statusAnterior]);
...
function filtrarCronograma() {
    ...
    linhas.forEach(tr => {
        ...
        if (descRow && descRow.classList.contains('tr-descricao') && !ok) {
            ...
            const expandBtn = tr.querySelector('.btn-expandir i');
            if (expandBtn) expandBtn.className = 'bi bi-three-dots-vertical';
        }
        if (ok) visiveis++;
    });
