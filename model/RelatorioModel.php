...
class RelatorioModel {
    ...
    public function usuariosPorPerfil() {
        return $this->fetch("SELECT perfil, COUNT(*) AS total FROM usuarios GROUP BY perfil ORDER BY total DESC");
    }

    public function topProjetosPorParticipantes($limite = 10) {
        return $this->fetch("
            SELECT p.titulo, COUNT(pa.id_participacao) AS total_participantes, p.status
            FROM projetos p
