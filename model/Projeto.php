...
class Projeto
{
    ...
    {
        try {
            $sql = "INSERT INTO projetos (titulo, area, id_tipo, descricao, data_inicio, data_fim, status) 
                VALUES (:titulo, :area, :id_tipo, :descricao, :data_inicio, :data_fim, :status)";

            $stmt = $this->pdo->prepare($sql);

            // Executamos vinculando os valores do array $dados
            return $stmt->execute([
                ':titulo'      => $dados['titulo'],
                ':area'        => $dados['area'],
                ...
                ':status'      => 'ativo' // Definimos como ativo por padrão
            ]);
